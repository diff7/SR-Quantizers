...
class FlopsLoss:
    def __init__(self, flops, penalty):
        self.scale = flops.detach().item()
    ...
    def __call__(self, weighted_flops):
        return weighted_flops * self.penalty / self.scale
...
def get_data_loaders(cfg):
    ...
    if cfg.dataset.debug_mode:
        cfg.dataset.train_sample = 0.0001

    split = int(cfg.dataset.train_sample * n_train * 0.7)
    ...
    valid_sampler = torch.utils.data.sampler.SubsetRandomSampler(
        indices[split:] if not cfg.dataset.debug_mode else indices[:split]
    )
