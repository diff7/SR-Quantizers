...
class IMDModule(nn.Module):
    ...
    def forward(self, input):
        ...
        out = torch.cat(
            [distilled_c1, distilled_c2, distilled_c3, out_c4], dim=1
        )
...
class IMDModule_Large(nn.Module):
    def __init__(self, in_channels, distillation_rate=1 / 4):
        ...
        self.remaining_channels = int(
            in_channels - self.distilled_channels
        )  # 18
        self.c1 = conv_layer(
            in_channels, in_channels, 3, bias=False
        )  # 24 --> 24
        self.c2 = conv_layer(
            self.remaining_channels, in_channels, 3, bias=False
        )  # 18 --> 24
        self.c3 = conv_layer(
            self.remaining_channels, in_channels, 3, bias=False
        )  # 18 --> 24
        self.c4 = conv_layer(
            self.remaining_channels, self.remaining_channels, 3, bias=False
        )  # 15 --> 15
        self.c5 = conv_layer(
            self.remaining_channels - self.distilled_channels,
            self.remaining_channels - self.distilled_channels,
            3,
            bias=False,
        )  # 10 --> 10
        self.c6 = conv_layer(
            self.distilled_channels, self.distilled_channels, 3, bias=False
        )  # 5 --> 5
        self.act = activation("relu")
        self.c7 = conv_layer(
            self.distilled_channels * 6, in_channels, 1, bias=False
        )
    ...
    def forward(self, input):
        ...
        distilled_c5, remaining_c5 = torch.split(
            ...
            (
                self.distilled_channels,
                self.remaining_channels - self.distilled_channels * 2,
            ),
            dim=1,
        )  # 6, 6
